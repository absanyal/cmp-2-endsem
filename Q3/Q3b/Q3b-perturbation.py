...
for k in kl:
    ...
    k_spectrum_b0.append([k, np.real(eVal[1]), eVec[1]])
    evl.append(eVal[0])
...
def fd(k1_i, k2_i, x1_i, x2_i):
    return np.real(np.conjugate(u(k1_i, x1_i)) * np.conjugate(u(k2_i, x2_i)) \
    * u(k1_i, x1_i) * u(k2_i, x2_i) \
    * 1/( (x1_i - x2_i ) + h/2 )) / (4 * np.pi)

def fe(k1_i, k2_i, x1_i, x2_i):
    return np.real(np.conjugate(u(k1_i, x1_i)) * np.conjugate(u(k2_i, x2_i)) \
    * u(k1_i, x2_i) * u(k2_i, x1_i) \
    * 1/( (x1_i - x2_i ) + h/2 )) \
    * np.exp( I * ( k1_i - k2_i ) * ( x1_i - x2_i ) ) / (4 * np.pi)

#J_k = []
#K_k = []
#for k1_i in range(atoms-1):
#    J = 0
#    K = 0
#    print(k1_i)
#    for k2_i in range(atoms - 1):
#        for x1_i in range(len(xn)):
#            for x2_i in range(len(xn)):
...
#        for x2_i in range(len(xn)):
#            for k2_i in range(atoms-1):
#                J += fe(k1_i, k2_i, x1_i, x2_i) * (1 - kdel(k1_i, k2_i))
#    K_k.append(J)
